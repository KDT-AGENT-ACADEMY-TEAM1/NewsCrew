...
def get_flat_categories() -> list[dict]:
    ...
    try:
        ...
        rows = fetch_all(
            "SELECT c.id, c.code, c.name, c.keywords, p.name AS parent_name "
...
def list_categories() -> list[dict]:
    ...
    from .db import fetch_all
    rows = fetch_all(
        "SELECT c.id, c.code, c.name, c.keywords, c.sort_order, c.is_active, "
...
def create_category(
    ...
    keywords 는 JSON 컬럼이라 json.dumps 로 문자열로 만들어 넣습니다.
    code 가 이미 있으면 UNIQUE 제약으로 예외가 납니다(중복 방지).
    """
    from .db import execute
    return execute(
        "INSERT INTO interest_category "
        "(parent_id, code, name, description, keywords, sort_order) "
        "VALUES (%s, %s, %s, %s, %s, %s)",
        (
            parent_id,
            code,
